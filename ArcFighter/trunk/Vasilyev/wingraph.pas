...
procedure PutImage(x1,y1:smallint; bitmap: pointer; bit:word); overload;
procedure PutImage(x1,y1:smallint; bitmap: pointer); overload;
procedure PutImageTransparent(x,y:smallint; bitmap: pointer; transColor:longword); overload;
procedure PutImageTransparent(x,y:smallint; bitmap: pointer);  overload;
...
procedure PutImage(x1,y1:smallint; bitmap: pointer);
begin
	PutImage(x1,y1, bitmap, NormalPut);
end;

procedure PutImage(x1,y1:smallint; bitmap: pointer; bit:word);
var hbmp: HBITMAP;
    img : ^TImage;
    rop : DWORD;
begin
...
procedure PutTempImageTransparent(x,y:smallint; width, height: smallint; transColor:longword);
var
	ix, iy: smallint;
	icolor: longword;
begin
	...
	begin				
			icolor := windows.GetPixel(grTemp, ix, iy);
			if (icolor <> transColor) then
			begin
				if grDirect then
					windows.SetPixelV(grWindow, x+ix, y+iy, icolor);

				windows.SetPixelV(grMemory, x+ix, y+iy, icolor);
			end;				
		end;
	end;
end;

procedure PutImageTransparent(x,y:smallint; bitmap: pointer; transColor:longword);
var
	hbmp: HBITMAP;
	img : ^TImage;
begin
	...
	begin
		...
		PutTempImageTransparent(x,y, biWidth, biHeight, transColor);
...
begin
	...
	begin
		...
		icolor := windows.GetPixel(grTemp, 0, 0);

		PutTempImageTransparent(x,y, biWidth, biHeight, icolor);	
